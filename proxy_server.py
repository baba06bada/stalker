...
Stream akisi:
  1. Browser /tslive?cmd=... -> create_link ile MPEG-TS URL al
  2. Proxy MPEG-TS stream'ini HTTP chunked olarak <video>'ya iletir
  3. TV hardware decoder ile direkt oynatir (hls.js/MSE gerekmez)

Xtream icin: /tsproxy?url=ENCODED_STREAM_URL
"""
import http.server
...
class StreamSession:
    ...
    def _parse_packet(self, pkt):
        ...
        return {'pid': pid, 'pusi': pusi, 'has_adapt': has_adapt, 'has_payload': has_payload, 'raw': pkt}
    ...
    def _parse_pmt(self, pkt):
        ...
        while idx + 5 <= len(pay) and idx + 5 <= sec_end:
            ...
            if stype == 0x1b:
                ...
                if vpid is None:
                    vpid = epid
            elif stype in (0x0f, 0x11, 0x03, 0x04):
                if apid is None:
                    apid = epid
        return vpid, apid
